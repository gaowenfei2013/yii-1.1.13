...
class CMap extends CComponent implements IteratorAggregate,ArrayAccess,Countable
{
	...
	// 递归合并2个或2个以上的数组到一个数组
	// 这个方法同PHP自带的array_merger_recursive是不一样的
	public static function mergeArray($a,$b)
	{
		...
		while(!empty($args))
		{
			...
			foreach($next as $k => $v)
			{
				if(is_integer($k)) // 如果是整形
					isset($res[$k]) ? $res[]=$v : $res[$k]=$v; // 如果已经存在，后移一位，否则就已这个整形为键名
				elseif(is_array($v) && isset($res[$k]) && is_array($res[$k])) // 如果两边都是数组
					$res[$k]=self::mergeArray($res[$k],$v); // 递归调用
				else
					$res[$k]=$v;
			}
		}
	...
	// 下面的offset开头的方法 实现了ArrayAccess接口
