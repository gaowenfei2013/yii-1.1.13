...
class CHttpRequest extends CApplicationComponent
{
	 ...
	 */
	public $enableCookieValidation=false; // 是否需要验证cookie没有被篡改 默认不需要验证
	/**
	...
	// 是否需要启动CSRF（跨站请求伪造）验证。默认不需要
	// 如果设置为true，表单提交到一个Web应用程序必须是相同的应用程序，如果不是，提示400
	// 注意，这个特性需要客户端cookie支持
	// 你还需要使用CHtml::form 或 CHtml::statefulForm来生成表单html，也就是必须使用这2个才起作用
	...
	{
		// 如果开启了magic_quotes_gpc，首先需要规范去掉多余的反斜杠
		// normalize request
		...
		// 向onBeginRequest事件添加事件句柄，用来验证csrf
	...
	// 返回应用url的schema and host部分
	// 例如 http://localhost 最后没有 / 符号的
	// 默认是根据当前请求信息确定 hostinfo
	// 当然你可以显示的通过 setHostInfo 进行设置
	public function getHostInfo($schema='')
	{
		if($this->_hostInfo===null)
		{
			if($secure=$this->getIsSecureConnection()) // 是否安全链接
				$http='https';
			else
				$http='http';
	...
	// 返回入口脚本的相对url 就是返回带入口脚本的相对url
	// 比如当前请求为 http://localhost/yii/angel/backend.php/site/index
	// 方法返回 /yii/angel/backend.php
	...
	// 返回当前请求的URL路径信息
	// 例如当前请求 http://localhost/yii/angel/backend.php/site/index
	// 这个方法返回 site/index
	...
	// 返回为当前请求的URL的uri部分
	// 比如当前请求 http://localhost/yii/angel/backend.php/site/index
	// 这个方法返回 /yii/angel/backend.php/site/index
	...
	// 判断是否Adobe Flash or Adobe Flex发过来的请求
	...
	// 返回安全请求使用的端口 默认443 安全请求就是https请求
	...
	// 重定向浏览器到指定的url
	// 注意，如果$url以 / 开头，那么将相对于当前的请求url
	// $terminate 是否要终止当前请求
	// $statusCode 重定向使用的http状态码 默认302
	...
	// 返回一个数组，用户接受语言的优先顺序
	// 如果Accept-Language:zh-CN,zh;q=0.8
	// 将返回array('zh-CN', 'zh')
	...
	{
		// 如果是POST、PUT、DELETE请求之一，才需要验证
		if ($this->getIsPostRequest() ||
			$this->getIsPutRequest() ||
			$this->getIsDeleteRequest())
		{
			$cookies=$this->getCookies(); //所有cookie

			$method=$this->getRequestType(); // 请求类型
			switch($method)
			{
				case 'POST':
...
// CCookieCollection实现了用于保存cookie的一个集合
// 在request的getCookies方法中使用了改类
class CCookieCollection extends CMap
{
	private $_request; // 保存当前请求组件实例
	private $_initialized=false; // 是否初始化过
	...
	// 返回通过验证的cookie列表 一个数组
	protected function getCookies()
	{
		$cookies=array();
		if($this->_request->enableCookieValidation) // 如果需要验证cookie
		{
			$sm=Yii::app()->getSecurityManager(); // 安全组件
	...
	{
		if($cookie instanceof CHttpCookie) // 必须是chttpcookie的实例
		{
			$this->remove($name);
	...
	{
		if(($cookie=parent::remove($name))!==null) // 如果清除掉了 这里调用父类的remove 是为了删除掉$_data中的数据
		{
			if($this->_initialized)
			{
				$cookie->configure($options); // 配置cookie的属性
				$this->removeCookie($cookie); // 删除cookie
			}
		}
	...
	{
		$value=$cookie->value;
		if($this->_request->enableCookieValidation) // 如果需要验证cookie 也就是对cookie进行安全加密
