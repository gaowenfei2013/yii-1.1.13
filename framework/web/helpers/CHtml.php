...
class CHtml
{
	 ...
	 */
	// 创建一个html标签
	// $tag 标签名，如 a 就表示创建 <a……>……</a>
	// $htmlOptions 表示这个标签的属性
	// $content 标签的内容（它不会被编码，值是什么就是什么）
	// $closeTag 表示是否产生结束标记
	 ...
	 */
	// 给字符串包围一个 CDATA 标记 （一般用于 xml 、css、javascript）
	 ...
	 */
	// 创建一个 meta 标签
	// $content 标签的 content 属性
	// $name 标签的 name 属性
	// $httpEquiv 标签的 http-equiv 属性
	// $options 其他属性
	...
	// 创建一个开始 form 标签
	// $action 表单的action属性
	// $method 表单提交的方法，method属性
	// $htmlOptions 附加在表单上的其他属性
	...
	public static function beginForm($action='',$method='post',$htmlOptions=array())
	{
		...
		// 如果不是get请求，并且提交的url中函数其他参数
		// 将参数提取出来以 input hidden 标签形式 加入到 form 中间
	 ...
	 */
	// 创建一个有状态的表单
	// 有状态的表单和普通表单类似，只是多了个保存页面状态的隐藏字段
	...
	{
		if($url!=='')
			$htmlOptions['href']=self::normalizeUrl($url); // 规范化url
	 ...
	 * @see clientChange
	 */
	// 创建一个按钮，如 <input type="button" name="button" value="button" />
	// $label 显示的按钮标签
	// $htmlOptions 其他属性
	public static function button($label='button',$htmlOptions=array())
	{
		// 如果没有指定 name 属性，自动创建一个
	 ...
	 * @see clientChange
	 */
	// 创建一个使用 html 标签的按钮，如 <button>button</button>
	public static function htmlButton($label='button',$htmlOptions=array())
	{
		// 如果没有设置 name 属性，自动创建一个
		if(!isset($htmlOptions['name']))
			$htmlOptions['name']=self::ID_PREFIX.self::$count++;
