...
{
	const FLASH_KEY_PREFIX='Yii.CWebUser.flash.'; // 保存flash使用的key前缀
	const FLASH_COUNTERS='Yii.CWebUser.flashcounters'; // 保存闪存信息的计数器，比如某个名称的闪存计数器不为零，就删除掉
	 ...
	 */
	public $allowAutoLogin=false; // 是否允许基于cookie的自动登录
	/**
	 ...
	 */
	public $loginUrl=array('/site/login'); // 登录地址
	/**
	 ...
	 */
	// 每次请求是否续约保存身份的cookie
	// 默认值为false，这个属性只在允许自动登录下有效
	// 当设置为false，保存身份的cookie就会在指定的时间后过期
	 ...
	 */
	// 是否自动更新闪存消息的有效性
	// 默认true，意味着闪存信息将只适用于当前和下一个请求
	// 如果设置为false，你需要确保使用闪存消息后要删除掉
	 ...
	 * @see loginRequired
	 */
	// 一个字符串，在ajax调用的时候、当用户session过期时输出
	public $loginRequiredAjaxResponse;

	private $_keyPrefix;
	private $_access=array(); // 保存访问检查的缓存信息
	...
	public function init()
	{
		...
		// 如果是来宾，并且可以从cookie自动登录
		if($this->getIsGuest() && $this->allowAutoLogin)
			$this->restoreFromCookie(); // 从cookie恢复认证信息
		elseif($this->autoRenewCookie && $this->allowAutoLogin) // 如果允许自动更新身份cookie，并且允许自动登录，这个将保存身份的cookie延期
			$this->renewCookie(); // 更新身份cookie
		if($this->autoUpdateFlash) // 如果自动更新闪存数据
			$this->updateFlash(); // 更新闪存数据

		$this->updateAuthStatus(); // 更新认证状态
	}
	 ...
	 */
	// 登录
	// 用户的认证信息将被保存到session中。默认情况下，是保存到session中的。如果过期时间duration大于0，那么一个cookie将被发送到客户端以便下次登录。
	public function login($identity,$duration=0)
	{
		$id=$identity->getId(); // 用户的id
		$states=$identity->getPersistentStates(); // 所有身份信息
		if($this->beforeLogin($id,$states,false))
		{
			$this->changeIdentity($id,$identity->getName(),$states); // 更新身份信息

			if($duration>0) // 如果过期时间大于0，也就是所谓的记住我，需要allowAutoLogin支持
			{
				if($this->allowAutoLogin)
					$this->saveToCookie($duration); // 将身份信息保存到cookie
				else
	...
	public function logout($destroySession=true)
	{
		if($this->beforeLogout())
		{
			if($this->allowAutoLogin)
			{
				...
				if($this->identityCookie!==null) // 又新建保存身份的cookie
				{
			...
			if($destroySession) // 删除全部session数据
				Yii::app()->getSession()->destroy();
			else
				$this->clearStates(); // 清楚掉保存在session中的身份信息
			$this->_access=array(); // 清除访问检查信息
			$this->afterLogout();
		}
	}
	 ...
	 */
	// 跳转到登录页
	// 在跳转前，会记录当前的url（如果不是ajax调用），所以在登录成功后，会跳转回这里记录的url。
	// 确保你设定的loginUrl能正确的跳转到登录页面。
	// 在调用这个方法后，当前请求将被终止，也就是exit了。
	public function loginRequired()
	{
		$app=Yii::app();
		$request=$app->getRequest();

		if(!$request->getIsAjaxRequest()) // 如果不是ajax请求
			$this->setReturnUrl($request->getUrl()); // 记录url，一边跳转回来
		elseif(isset($this->loginRequiredAjaxResponse)) // 如果是ajax请求，并且设置了loginRequiredAjaxResponse
		{
			echo $this->loginRequiredAjaxResponse;
			Yii::app()->end();
		}

		if(($url=$this->loginUrl)!==null) // 如果设置了loginUrl
		{
			if(is_array($url))
			{
	...
	// 这个方法将在用户登录之前被调用
	// 你可以重写此方法来提供额外的检查
	// 例如，当设置为可以根据cookie自动登录的情况下，你可能想要验证用户的id和一个随即的token。
	// 这样可以阻止黑客伪造cookie，即使他们获取了服务器的私钥。
	 ...
	 */
	// 这个方法将在用户成功登录后被调用
	// 你可以重写这个方法来做一些处理，比如，记录登录的ip、时间，加载用户的权限信息
	 ...
	 */
	// 这个方法将在登出之前被调用
	// 如果这个方法返回false，登出动作将被取消。
	// 你可以重写此方法来在注销用户之前做一些其他检查
	 ...
	 * @see saveToCookie
	 */
	// 从cookie中恢复当前用户信息，也就是从cookie登录。
	// 这个方法适用在运行自动登录的情况下
	// 用户身份信息从cookie中恢复
	// 足够的安保措施是为了cookie不被篡改
	protected function restoreFromCookie()
	{
		$app=Yii::app();
		$request=$app->getRequest();
		$cookie=$request->getCookies()->itemAt($this->getStateKeyPrefix()); // 身份信息保存的cookie
		if($cookie && !empty($cookie->value) && is_string($cookie->value) && ($data=$app->getSecurityManager()->validateData($cookie->value))!==false) // 验证cookie
		{
			...
			if(is_array($data) && isset($data[0],$data[1],$data[2],$data[3]))
			{
				...
				{
					$this->changeIdentity($id,$name,$states); // 更新身份状态
					if($this->autoRenewCookie) // 将保存身份信息的cookie延期
					{
	...
	{
		$keys=array_keys($_SESSION); // 找出所有的session  key
		$prefix=$this->getStateKeyPrefix(); // 获取状态键前缀
		$n=strlen($prefix);
		foreach($keys as $key) // 根据前缀循环清除
		{
	 ...
	 */
	// 返回所有闪存数据
	// 这个方法类似于getFlash，不同的是它返回所有目前可用的闪存数据
	 ...
	 */
	// 获取一个指定名称的闪存信息
	// 一个闪存信息只适用于当前请求和下一个请求，因为返回后就通过第三个参数删除掉了
	...
	{
		Yii::app()->getSession()->regenerateID(true); // 更新sesion id
		$this->setId($id); // 设置id
		$this->setName($name); // 设置用户名
		$this->loadIdentityStates($states); // 保存身份状态信息
	}
	...
	protected function loadIdentityStates($states)
	{
		...
		$this->setState(self::STATES_VAR,$names); // 将所有的身份信息的名称一同保存起来
	}
	...
	{
		$counters=$this->getState(self::FLASH_COUNTERS); // 计数器
		if(!is_array($counters))
			return;
		foreach($counters as $key=>$count)
		{
			if($count) // 如果这个key的闪存计数器不为0，那么删除之
			{
				unset($counters[$key]);
	...
	// 根据authTimeout更新认证状态
	// 如果这个用户已经无效，那么将被注销
	// 这个方法是在user组件初始化时主动调用的
	protected function updateAuthStatus()
	{
		// 如果设置了超时时间，并且不是访客
		if($this->authTimeout!==null && !$this->getIsGuest())
		{
			$expires=$this->getState(self::AUTH_TIMEOUT_VAR); // 超时的时刻
			if ($expires!==null && $expires < time()) // 如果当前时刻已经超过超时时刻，注销当前用户
				$this->logout(false);
			else // 否则将超时时刻往后延长authTimeout
