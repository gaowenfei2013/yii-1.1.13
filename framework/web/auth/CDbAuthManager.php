...
class CDbAuthManager extends CAuthManager
{
	...
	private $_usingSqlite; // 标志是否使用sqlite
	...
	{
		$assignments=$this->getAuthAssignments($userId); // 返回用户拥有的授权项
		return $this->checkAccessRecursive($itemName,$userId,$params,$assignments); // 递归的检查
	}
	...
	{
		if(($item=$this->getAuthItem($itemName))===null) // 查找出授权项，没有直接返回false
			return false;
		Yii::trace('Checking permission "'.$item->getName().'"','system.web.auth.CDbAuthManager');
		if(!isset($params['userId']))
		    $params['userId'] = $userId;
		if($this->executeBizRule($item->getBizRule(),$params,$item->getData())) // 首先执行其他业务规则的检查
		{
			if(in_array($itemName,$this->defaultRoles)) // 从默认角色中检查
				return true;
	...
	{
		if($type===null && $userId===null) // 所有授权项
		{
			$command=$this->db->createCommand()
				->select()
				->from($this->itemTable);
		}
		elseif($userId===null) // 指定类型的所有授权项
		{
			$command=$this->db->createCommand()
				->select()
				->from($this->itemTable)
	...
	{
		if($this->usingSqlite()) // 如果使用的是sqlite数据库
		{
			$this->db->createCommand()
