...
class CController extends CBaseController
{
	...
	{
		if(($action=$this->createAction($actionID))!==null) // 创建动作成功
		{
			if(($parent=$this->getModule())===null) // 判断这个控制器属于哪个模块，没有的话就是当前应用
				$parent=Yii::app();
			...
			$this->missingAction($actionID); // 找不到动作
	}
	...
	{
		if(empty($filters)) // 没有过滤器 直接执行动作
			$this->runAction($action);
		else
		{
			$priorAction=$this->_action;
			$this->_action=$action;
			CFilterChain::create($this,$action,$filters)->run(); // 执行过滤器
	 ...
	 */
	// 对视图渲染后的结果做后期处理
	// 比如，注册了客户端脚本，这个方法会在输出内容的相应位置插入脚本
	// 如果有动态输出，也是在这里处理。
	// 这个方法也可能将页面状态保存到表单的hidden元素中
	public function processOutput($output)
	{
		...
		{
			$output=$this->processDynamicOutput($output); // 执行替换动态内容
	 ...
	 */
	// 处理动态输出
	// 这个方法是yii内部使用的，不要主动调用这个方法
	public function processDynamicOutput($output)
	{
		if($this->_dynamicOutput) // 如果存在片段缓存
		{
			// 标志替换
	...
	public function createAction($actionID)
	{
		...
		// 内部动作
		// strcasecmp($actionID,'s') 不能定义为actions，因为actions方法已经被控制器默认使用了
	 ...
	 */
	// 根据动作映射表创建动作
	// 这个方法将检查action id是否存在于action map中。如果存在，相应的配置将用于创建动作实例。
	// $actionMap action map 就是控制器 actions 返回的数组
	// $actionID 动作的ID
	// $requestActionID 原始的动作ID
	// $config 配置信息
	protected function createActionFromMap($actionMap,$actionID,$requestActionID,$config=array())
	{
		// 如果动作ID中找不到 . 号，并且这个动作ID在 map 中找到
		if(($pos=strpos($actionID,'.'))===false && isset($actionMap[$actionID]))
		{
			// 动作映射配置可以是个数组，也可是直接是个值（动作类的文件路径）
		...
		// 这个动作定义在另一个提供者类中
		// 例如 /user/a.update 此时 $actionID = a.update
		// 我们会在当前 action map中查找有没有 a 键名的定义
		// 例如：return array('a' => 'path.to.a')
		// 这时候，yii会调用a类中的actions方法，在这个action map中继续寻找actionID的动作
	 ...
	 */
	// 返回控制器的ID，如果是模块下的的控制器，会以模块id为前缀，例如 a/b
	 ...
	 * if the controller does not belong to any module
	 */
	// 返回控制器所属的模块，如果没有返回null，例如直属于 app
	...
	// $view 视图文件名
	// $data 传递给视图的数据
	// $return 是否返回渲染后的数据，默认false，表示直接输出
	 ...
	 */
	// 此方法在视图渲染后调用
	// 注意这个方法是在 processOutput 方法前调用的
	// 你可能需要重写此方法，对视图渲染结果做一些处理
	// $view 视图文件名
	// $output 视图渲染后的结果
	 ...
	 * @since 1.1.8
	 */
	// 渲染一个指定名称的剪辑，可以使用 $params 提供的数组，进行替换剪辑的内容
	 ...
	 */
	// 为这个控制器指定的动作创建一个相对url
	// $route 路由
	// $params $_GET参数
	// $schema http或者https，如果为空使用当前url的schema
	// $ampersand url参数连接符号，默认 &
	 ...
	 */
	// 记录缓存动作
	// $context 控制器的属性，指向一个对象，表示谁的方法被调用
	// $method 方法名
	// $params 传递给的参数
	...
	// 返回一个页面状态值
	// 一个页面状态是指保存的$_POST值
	// 如果希望使用这个功能，表单必须使用 CHtml::statefulForm 创建
	public function getPageState($name,$defaultValue=null)
	{
		if($this->_pageStates===null)
			$this->_pageStates=$this->loadPageStates(); // 加载
		// 返回
