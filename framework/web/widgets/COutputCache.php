...
// 输出缓存
// 缓存一个动作或者片段
// 缓存一个动作，就是将其作为动作的过滤器使用
// 缓存片段，beginCache endCache
class COutputCache extends CFilterWidget
{
	...
	{
		if(!$this->getIsContentCached()) // 缓存已经失效，继续执行过滤器链，进而执行动作
			$filterChain->run();
		$this->run();
	}
	...
	public function run()
	{
		if($this->getIsContentCached()) // 如果可以找到缓存
		{
			if($this->getController()->isCachingStackEmpty()) // 如果缓存栈不为空，说明还有片段缓存要处理
