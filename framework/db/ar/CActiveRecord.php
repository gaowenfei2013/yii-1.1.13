...
{
	// 一般从数据库角度讲，有一对多、多对一、多对多关系，下面是ar类的关系映射
	// 
	// 属于，如果A和B之间是一对多关系，那么B属于A，例如Post属于User
	const BELONGS_TO='CBelongsToRelation';
	// 有一个，这个是HAS_MANY的一个特例，A最多有一个B，例如User最多有一个Profile
	const HAS_ONE='CHasOneRelation';
	// 有多个，如果表A和B之间是一对多关系，那么A则有多个B，例如User有多个Post
	const HAS_MANY='CHasManyRelation';
	// 对应于数据库中的多对多关系，由于多数数据库不支持多对多关系，因此需要一个关联表将多对多关系分割为一对多关系
	 ...
	 */
	// 构造，在这里指定场景 scenario ，默认场景是insert
	public function __construct($scenario='insert')
	{
		...
		$this->setIsNewRecord(true); // 标记这条记录是新添加的
		$this->_attributes=$this->getMetaData()->attributeDefaults; // 数据库字段有默认值的字段和默认值组成的数组
	 ...
	 */
	// 返回与此模型相关的查询条件
	// $createIfNull 表示是否当查询条件实例不存在的时候自动创建
	 ...
	 */
	// 返回相关数据库表的主键
	// 这个方法是为了如果表没有定义主键时需要你进行覆盖的，如果有主键，不需要你做任何事情，会自动查询数据库得到
	 ...
	 */
	// 不要主动调用这个方法。这个方法时内部使用，用来填充相关对象的。
	...
	public function insert($attributes=null)
	{
		...
		if($this->beforeSave())
		{
			...
			if($command->execute())
			{
				...
				$this->afterSave();
				$this->setIsNewRecord(false); // 切换到更新
	 ...
	 * @since 1.1.8
	 */
	// 保存计数器，比如表中有个view_count，你想递增这个字段，就用这个方法array(字段名=>递增的数字)
	 ...
	 */
	// 设置当前活动记录的主键值
	// 当调用了这个方法后，可以调用oldPrimaryKey()返回之前的主键值
	 ...
	 * @see saveCounters
	 */
	// 更新一个或多个计数器，就是递增例如view_count这样的字段
...
class CActiveRecordMetaData
{
	 ...
	 */
	public $relations=array(); // 关系映射列表
	/**
	 * @var array attribute default values
	 */
	public $attributeDefaults=array(); // 属性的默认值
