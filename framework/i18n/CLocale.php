...
class CLocale extends CComponent
{
	 ...
	 * @since 1.1.0
	 */
	// 区域数据的保存路径，默认 framework/i18n/data
	 ...
	 */
	// 构造函数
	// 由于 protected 所以不能直接 new 实例化，需要使用 getInstance
	protected function __construct($id)
	{
		$this->_id=self::getCanonicalID($id); // 规范id，并保存
		// 确定数据保存目录
		$dataPath=self::$dataPath===null ? dirname(__FILE__).DIRECTORY_SEPARATOR.'data' : self::$dataPath;
		...
		// 存在，将数据保存如 $this->_data 变量，否则抛出异常
		if(is_file($dataFile))
